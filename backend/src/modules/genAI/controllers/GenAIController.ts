...
export class GenAIController {
  ...
  @OpenAPI({
    summary: 'Proxy temporary files',
    description: 'Proxy temp files generated by AI server (transcripts, audio, json).',
  })
  ...
  async getTempFile(@Params() params: { fileId: string }, @Res() res: ExpressResponse) {
    ...
    try {
      ...
      console.warn('[GenAIController] Proxy fetch to AI server failed, using local disk fallback...', err);
    }

    try {
      const fs = await import('fs');
      const path = await import('path');
      const cleanName = fileId.replace(/^local_/, '');
      const candidatePaths = [
        path.resolve(process.cwd(), '../vibe-ai/temp_storage', cleanName),
        path.resolve(process.cwd(), '../vibe-ai/temp_audio', cleanName),
        path.resolve(process.cwd(), 'vibe-ai/temp_storage', cleanName),
      ];
      ...
      console.error('[GenAIController] Disk fallback error:', diskErr);
    }
