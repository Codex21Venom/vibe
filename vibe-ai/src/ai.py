...
async def start_transcript_generation_task(job_id: str, file: str, approval_data: TranscriptParameters) -> Dict[str, Any]:
    ...
    try:
        ...
        if not transcript_file_url:
            ...
            local_path = os.path.join(temp_dir, f"{job_id}_{run_id}_transcript.json")
            with open(local_path, "w", encoding="utf-8") as f:
                json.dump(transcript, f)
            transcript_file_url = local_path
...
async def start_segmentation_task(job_id: str, file: str, approval_data: Optional[SegmentationParameters] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        transcript = None
        if file:
            # Download or read the transcript file
            print(f"Retrieving transcript from: {file}")
            try:
                if str(file).startswith("http://") or str(file).startswith("https://"):
...
async def start_question_generation_task(job_id: str, segmentMap, file, approval_data: Optional[QuestionGenerationParameters] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            if not questions_file_url:
                # Fallback to local storage
                temp_dir = os.path.join(os.path.dirname(__file__), "temp_audio")
                os.makedirs(temp_dir, exist_ok=True)
                local_path = os.path.join(temp_dir, f"{job_id}_{run_id}_questions.json")
                with open(local_path, "w", encoding="utf-8") as f:
                    json.dump(questions, f)
                questions_file_url = local_path            
